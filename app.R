  ...
  tabPanel(
    ...
    fluidPage(
      div(
        ...
        textInput("search", "", placeholder = "buscar...", width = "720px"),
      ),
      div(
        style = "justify-content: center; display: flex; max-width: 720px; margin-inline: auto; padding-bottom: 5rem;",
...
renderShowTable <- function(output, session, WindowIndex) {
  if (nrow(showDataFrame) > 1 && tipo_dmu != 'tabela') {
    show("oculDEA")
    show("oculButton")
...
processDEA <- function(input, output) {
  ...
  for (windowIndex in 1:length(timeSeriesResultTable)) {
    ...
    new_col_name <- sprintf("%s", windowIndex)

    len_combined <- nrow(combinedTableDea)
    len_table <- length(tableDea[[2]])
    print(len_combined)
    print(len_table)
    if (len_table < len_combined) {
      na_fill <- data.frame(matrix(NA, nrow = len_combined - len_table, ncol = ncol(tableDea[[2]])))
      colnames(na_fill) <- colnames(tableDea[[2]])
      tableDea[[2]] <- rbind(tableDea[[2]], na_fill)
    } else if (len_table > len_combined) {
      na_fill_combined <- data.frame(matrix(NA, nrow = len_table - len_combined, ncol = ncol(combinedTableDea)))
      colnames(na_fill_combined) <- colnames(combinedTableDea)
      combinedTableDea <- rbind(combinedTableDea, na_fill_combined)
    }
  ...
  # print('out')
  # print(output_list)
  max_length_input <- max(sapply(input_list, length))
  max_length_output <- max(sapply(output_list, length))

  input_list_padded <- lapply(input_list, function(x) {
    length(x) <- max_length_input
    return(x)
  })

  output_list_padded <- lapply(output_list, function(x) {
    length(x) <- max_length_output
    return(x)
  })

  # print(output_list_padded)

  media_input <- colMeans(do.call(rbind, input_list_padded), na.rm = TRUE)
  media_output <- rowMeans(do.call(rbind, output_list_padded), na.rm = TRUE)

  if (length(media_input) < length(media_output)) {
    media_input <- c(media_input, rep(NA, length(media_output) - length(media_input)))
  } else if (length(media_output) < length(media_input)) {
    media_output <- c(media_output, rep(NA, length(media_input) - length(media_output)))
  }

  media_input <- na.omit(media_input)
  media_output <- na.omit(media_output)

  min_length <- min(length(media_input), length(media_output))

  media_input <- media_input[1:min_length]
  media_output <- media_output[1:min_length]

  if (input$mod == "SCCR") {
    output$graficoDea <- renderPlot(dea.plot(media_input, media_output, RTS = "CRS", ORIENTATION = input$ori))
    output$graficoDeafrontier <- renderPlot(dea.plot.frontier(media_input, media_output, RTS = "CRS"))
  } else if (input$mod == "ADD") {
    output$graficoDea <- renderPlot(dea.plot(media_input, media_output, RTS = input$mod, ORIENTATION = "in-out"))
    output$graficoDeafrontier <- renderPlot(dea.plot.frontier(media_input, media_output, RTS = input$mod))
  } else {
